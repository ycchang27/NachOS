...
public class UserProcess {
	...
	public int readVirtualMemory(int vaddr, byte[] data, int offset, int length) {
		...
		int virtual = Machine.processor().pageFromAddress(vaddr); //Get the virtual page number from virtual address
		int addrOffset = Machine.processor().offsetFromAddress(vaddr); //Get the offset from virtual address
		
		pageTable[virtual].used = true; //Change the page at address to used
		
		int physical = pageTable[virtual].ppn; //get the physical page number
		int physPage = physical*pageSize + addrOffset; //Calculate the physical address
		
		if(physical < 0 || physPage >= Machine.processor().getNumPhysPages())
			return 0;
	...
	protected boolean loadSections() {
		...
		for (int s = 0; s < coff.getNumSections(); s++) {
			...
			for (int i = 0; i < section.getLength(); i++) {
				...
				// accessing pageTable with virtual page number and saving that value into entry
				// variable
	...
	private int handleRead(int fileDescriptor, int vaddr, int size) {
		...
		if ((fileDescriptor < 0 || fileDescriptor >= MAX_PROCESSES) || processList[fileDescriptor] == null) {
			return -1;
		}
	...
	/**
	 * Array of "processes", which can be traversed by an index called "file
	 * descriptor"
	 */
	private Process[] processList;

	/**
	 * A class that represents a process. Its variables are declared as public to
	...
	/**
	 * Finds the next available index to insert for processList. If the list is
	 * full, return -1.
	 */
	...
	/**
	 * Finds the Process's index that corresponds with file name. Returns -1, if not
	 * found.
	 */
